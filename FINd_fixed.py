...
class FINDHasherFixed:
	# Bug-fixed version of FINDHasher. Two fixes vs FINd.py:
	#   1. boxFilter row-major indexing uses cols (was: rows) — original
	#      crashes on portrait images and silently corrupts on landscape.
	#   2. findHash256FromFloatLuma window-size arguments un-swapped — original
	#      sized the vertical window by image width and vice versa.
	# Both fixes are no-ops on square inputs, so on the meme_images dataset
	# (55 971 of 55 972 images are square) numerical baselines are unchanged.
	...
	def findHash256FromFloatLuma(
		...
		# Fix 2: vertical window scales with height (numRows), horizontal with width (numCols).
		# Original swapped these — distorted smoothing for non-square inputs.
	...
	def boxFilter(cls,input,output,rows,cols,rowWin,colWin):
		...
		for i in range(0,rows):
			for j in range(0,cols):
				...
				# Fix 1: row-major index of (k,l) in a (rows x cols) buffer is k*cols+l.
				# Original used k*rows+l, which is only correct when rows == cols.
